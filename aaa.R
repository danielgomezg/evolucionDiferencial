...
DE <- function(datos, tamañoP, fMutacion, CR){
  ...
  pNew <- list()
  pNew <- vector("list", length = tamañoP)
  ...
  p <- setup(datos, p, numCluster, features, tamañoP, cantDatos)
  ...
  while (g <= numGeneraciones & stopDE == FALSE) {
    ...
    for(i in 1:tamañoP){
      ...
      individuosRandom <- sample(1:tamañoP, 6, replace = FALSE)
    ...
    if(g > (0.7 * numGeneraciones)){
      individuoActual <- bestIndividuo(datos, p, features, numCluster, tamañoP)
      individuoNew <- bestIndividuo(datos, pNew, features, numCluster, tamañoP)
  ...
  mejorSolucion <- bestIndividuo(datos, p, features, numCluster, tamañoP)
  #return(p)
